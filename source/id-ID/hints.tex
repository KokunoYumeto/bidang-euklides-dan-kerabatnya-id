...
\parbf{\ref{ex:pyth}.}
Use that $\triangle ADC\sim \triangle CDB$.
...
Apply \ref{thm:parallel} to show that $n\nparallel \ell$, so $n$ and $\ell$ meet at one point, say~$P$.
Draw a line $\ell'\perp n$ thru $P$.
By \ref{prop:perp-perp}, $\ell'\parallel m$.
Apply \ref{thm:parallel} again, to show that $\ell= \ell'$.

\parit{(\ref{ex:perp-perp:b}).} Applying \textit{(\ref{ex:perp-perp:a})}, we get $k\perp m$.
It remains to apply \ref{prop:perp-perp}.
...
First, reuse the argument in \ref{ex:2-reflections}.
Further, show that the composition of two indirect motions is direct and use that reflection across a line is indirect (see \ref{cor:reflection+angle}).
...
The points of intersection of these two lines with $(AB)$ trisect the segment $[AB]$.
...
\parbf{\ref{ex:pent}.}
Apply \ref{thm:3sum} or \ref{ex:|3sum|} twice and \ref{thm:isos} thrice.
...
\right.
$$
if $0<a\le b\le c\le a+b$.

\parbf{\ref{ex:line-coord}.}
Notice that $MA=MB$ if and only if
