...
%\raggedcolumns\setlength{\multicolsep}{-15mm}
\spell{\begin{multicols}{2}}{}

\chapter[Petunjuk]{\vspace*{-20mm}Petunjuk\vspace*{-10mm}}
\refstepcounter{chapter}
\setcounter{eqtn}{0}

\parbf{\ref{ex:dist-square}.} Periksa ketaksamaan segitiga untuk $A\z=0$, $B\z=1$, dan $C\z=2$.

\parbf{\ref{ex:d_1+d_2+d_infty}.} 
Periksa semua syarat dalam \ref{def:metric-space}.
Mari kita bahas ketaksamaan segitiga --- syarat-syarat lainnya jelas dengan sendirinya.

Misalkan $A=(x_A,y_A)$, $B=(x_B,y_B)$, dan $C=(x_C,y_C)$.
Definisikan 
\begin{align*}
x_1&=x_B-x_A, 
&
...
\le 
|x_1|+|y_1|+|x_2|+|y_2|.$$
Ketaksamaan terakhir berlaku karena $|x_1+x_2|\le |x_1|+|x_2|$ 
dan
$|y_1+y_2|\le |y_1|+|y_2|$.
...
sederhanakan,
kuadratkan sekali lagi, lalu sederhanakan kembali.
Akan diperoleh ketaksamaan berikut:
$$0
\le 
(x_1\cdot y_2-x_2\cdot y_1)^2,$$
yang ekuivalen dengan \ref{eq:trig-inq-d2}
dan jelas benar.
...
\end{align*}
Dengan demikian, \ref{eq:max-trig} terbukti.

\parbf{\ref{ex:4-triangle}.} Jumlahkan keempat ketaksamaan segitiga.

\parbf{\ref{ex:dist-preserv=>injective}.}
Jika $A\ne B$, maka $d_\mathcal{X}(A,B)>0$.
Karena $f$ mempertahankan jarak,
$$d_\mathcal{Y}(f(A),f(B))=d_\mathcal{X}(A,B).$$
Oleh karena itu, $d_\mathcal{Y}(f(A),f(B))>0$; jadi $f(A)\z\ne f(B)$.

\parbf{\ref{ex:motion-of-R}.}
Misalkan $f(0)=a$ dan $f(1)=b$.
Tunjukkan bahwa $b=a+1$ atau $b=a-1$.
Selain itu, $f(x)=a\pm x$ dan $f(x)=b\pm(x-1)$ berlaku secara bersamaan untuk setiap~$x$.

Misalkan $b=a+1$. 
Tunjukkan bahwa 
$f(x)=a+x$ untuk setiap~$x$.

Dengan cara yang sama, jika $b=a-1$, 
tunjukkan bahwa 
$f(x)=a-x$ untuk setiap~$x$.

\parbf{\ref{ex:d_1=d_infty}.} 
Tunjukkan bahwa pemetaan $(x,y)\mapsto (x+y,x-y)$ merupakan isometri  $(\mathbb{R}^2,d_1)\z\to (\mathbb{R}^2,d_\infty)$.
Anda perlu memeriksa apakah pemetaan ini bijektif dan mempertahankan jarak.

\parbf{\ref{ad-ex:motions of Manhattan plane}.} 
Pertama, buktikan bahwa \textit{dua titik $A=(x_A,y_A)$ dan $B\z=(x_B,y_B)$ pada bidang Manhattan memiliki titik tengah tunggal jika dan hanya jika $x_A=x_B$ atau $y_A=y_B$}; bandingkan dengan contoh pada \ref{sec:cong-triangles}. 

Gunakan pernyataan di atas untuk membuktikan bahwa
setiap gerak pada bidang Manhattan 
dapat ditulis dalam salah satu dari delapan bentuk berikut:
\begin{align*}
(x,y)&\mapsto (\pm x+a,\pm y+b)
\shortintertext{atau} 
(x,y)&\mapsto (\pm y+b,\pm x+a),
\end{align*}
untuk bilangan real tetap $a$ dan~$b$.
Dalam setiap kasus, terdapat 4 pilihan tanda, sehingga untuk suatu pasangan tetap $(a,b)$ terdapat 8 gerak yang berbeda.

\parbf{\ref{ex:y=|x|}.}
Misalkan tiga titik $A$, $B$, dan $C$ terletak pada satu garis.
Perhatikan bahwa dalam hal ini salah satu dari tiga ketaksamaan segitiga yang melibatkan titik-titik $A$, $B$, dan $C$ menjadi kesamaan.


Ambil $A=(-1,1)$, $B=(0,0)$, dan $C=(1,1)$.
Tunjukkan bahwa untuk $d_1$ dan $d_2$
semua ketaksamaan segitiga yang melibatkan titik-titik $A$, $B$, dan $C$ bersifat ketat.
Ini menunjukkan bahwa grafik tersebut bukan garis.

Untuk $d_\infty$, tunjukkan bahwa $(x,|x|)\mapsto x$ merupakan isometri dari grafik tersebut ke~$\mathbb{R}$.
Simpulkan bahwa grafik tersebut merupakan garis dalam $(\mathbb{R}^2,d_\infty)$.

\parbf{\ref{ex:line-motion}.}
Uraikan definisi garis dan gerak.

\parbf{\ref{ex:trig==}.}
Berdasarkan definisi sinar,
terdapat suatu isometri
$$f\:[P Q)\to [0,\infty)$$
sedemikian sehingga $f(P)=0$.
Berdasarkan definisi isometri, $P X=f(X)$ untuk setiap $X\z\in [P Q)$.
Dengan demikian, $P X=r$ jika dan hanya jika $f(X)=r$.
...
berarti bahwa $2\cdot\alpha\z=2\cdot k\cdot\pi$ untuk suatu bilangan bulat~$k$.
Oleh karena itu,
$\alpha=k\cdot\pi$.

Secara ekuivalen, $\alpha=2\cdot n\cdot \pi$ atau $\alpha=(2\cdot n+1)\cdot \pi$ untuk suatu bilangan bulat~$n$.
Dalam kedua kasus tersebut, secara berturut-turut diperoleh $\alpha\equiv 0$ dan $\alpha\equiv \pi$.

\parbf{\ref{ex:a+b==c}.}
Perhatikan bahwa $\gamma'=\alpha+\beta\in [0,2\cdot \pi]$.
Tunjukkan dan gunakan fakta bahwa $\gamma'\equiv\gamma$ $\Rightarrow$ $\gamma'=\gamma$.

\parbf{\ref{ex:dist-cont}.} \textit{(a).}
Berdasarkan ketaksamaan segitiga,
$|f(A')\z-f(A)|\le d(A',A)$.
Oleh karena itu, kita dapat memilih $\delta=\epsilon$.
...
&\le d(A',A)+d(B',B).
\end{align*}
Oleh karena itu, kita dapat memilih $\delta=\tfrac\epsilon2$.

\parbf{\ref{ex:comp+cont}.}
Ambil $A\in \mathcal{X}$ dan $B\in\mathcal{Y}$
sedemikian sehingga $f(A)=B$.

Ambil $\epsilon>0$.
Karena $g$ kontinu di $B$, terdapat bilangan positif $\delta_1$ sedemikian sehingga 
...
Karena $f$ kontinu di $A$, terdapat $\delta_2>0$ sedemikian sehingga 
...
\parbf{\ref{ex:isom-cont}.}
Periksa definisinya dengan $\delta=\epsilon$.

\parbf{\ref{ex:ncong}.} \textit{(a).}
Tunjukkan bahwa $A\mapsto B$, $B\mapsto A$, $C\mapsto C$, $D\mapsto D$ mendefinisikan suatu gerak pada ruang tersebut;
simpulkan bahwa $\triangle ABC\cong \triangle BAC$.

\parit{(b).} Misalkan $\triangle ABC\cong \triangle BCA$; maka terdapat gerak $m$ yang memetakan $A\mapsto B$, $B\mapsto C$, dan $C\mapsto A$.
Tunjukkan bahwa $m\:D\mapsto D$; hal ini menghasilkan suatu kontradiksi.
...
\parbf{\ref{ex:infinite}.} Berdasarkan Aksioma~\ref{def:birkhoff-axioms:0}, terdapat sekurang-kurangnya dua titik pada bidang.
Oleh karena itu, berdasarkan Aksioma~\ref{def:birkhoff-axioms:1},
pada bidang tersebut terdapat sebuah garis.
Untuk membuktikan \textit{(a)}, cukup perhatikan bahwa sebuah garis merupakan himpunan titik yang tak hingga.
Untuk membuktikan \textit{(b)}, gunakan pula Aksioma~\ref{def:birkhoff-axioms:2}.

\parbf{\ref{ex:[OA)=[OA')}.}
Berdasarkan Aksioma~\ref{def:birkhoff-axioms:1},
$(OA)=(OA')$.
Oleh karena itu, pernyataan tersebut setara dengan pernyataan berikut:

\textit{Andaikan $f\:\mathbb{R}\to \mathbb{R}$ merupakan transformasi isometrik pada garis bilangan real yang memetakan $0\mapsto 0$ dan suatu bilangan positif ke bilangan positif.
Maka $f$ merupakan pemetaan identitas.}

Pernyataan terakhir merupakan akibat dari \ref{ex:motion-of-R}.

\parbf{\ref{ex:[OA)=[OB)}.}
Berdasarkan \ref{lem:AOA=0},
$\measuredangle AOA=0$.
Tinggal terapkan Aksioma~\ref{def:birkhoff-axioms:2a}.

\parbf{\ref{ex:lineAOB}.}
Terapkan \ref{lem:AOA=0},
\ref{thm:straight-angle},
dan \ref{ex:2a=0}.

\parbf{\ref{ex:ABCO-line}.}
Berdasarkan Aksioma~\ref{def:birkhoff-axioms:2b},
$2\cdot\measuredangle BOC
\equiv 
2\cdot\measuredangle AOC\z-2\cdot \measuredangle AOB
\equiv 0$.
Berdasarkan \ref{ex:2a=0},
hal ini menyiratkan bahwa
$\measuredangle BOC$ bernilai $0$ atau~$\pi$.
Dalam kedua kasus tersebut, tinggal terapkan masing-masing \ref{ex:[OA)=[OB)} dan \ref{thm:straight-angle}.

\parbf{\ref{ex:infinite-number-of-lines}.}
Ambil dua titik $A$ dan $B$ yang keberadaannya dijamin oleh Aksioma~\ref{def:birkhoff-axioms:0}.

Pilih bilangan real $0<\alpha<\pi$.
Berdasarkan Aksioma~\ref{def:birkhoff-axioms:2a}, terdapat titik $C$ sedemikian sehingga $\measuredangle ABC\z=\alpha$.

Gunakan \ref{cor:degenerate=pi} untuk menunjukkan bahwa $\triangle ABC$ tidak degenerat.
...
\parbf{\ref{ex:refelection-of-line}.} Terapkan \ref{prop:point-reflection} dan \ref{ex:line-motion}.
...
Ambil $\alpha=\measuredangle AOB$
dan
$\beta=\measuredangle BOA$.
Perhatikan bahwa $\alpha=\pi$ jika dan hanya jika $\beta=\pi$.
Jika tidak, $\alpha=-\beta$.
Dengan demikian, hasil yang diminta diperoleh.

\parbf{\ref{ex:PP(PN)}.}
Ambil $\alpha=\measuredangle ABC$, $\beta=\measuredangle A'B'C'$.
Karena $2\cdot\alpha\equiv 2\cdot \beta$, \ref{ex:2a=0} menyiratkan bahwa
 $\alpha\equiv \beta$ atau $\alpha\equiv \beta+\pi$.
Dalam kasus kedua, kedua sudut memiliki tanda yang berlawanan, dan hal ini mustahil.

Karena $\alpha,\beta\in(-\pi,\pi]$, kesamaan $\alpha\equiv \beta$ menyiratkan $\alpha= \beta$.

\parbf{\ref{ex:between}.}
Terapkan \ref{thm:signs-of-triug} pada $\triangle PAB$, $\triangle PBC$, dan $\triangle PAC$.

Untuk bagian kedua, gunakan penalaran seperti pada \ref{ex:a+b==c} untuk menunjukkan bahwa jika tiga bilangan $\alpha,\beta,\gamma\in (-\pi,\pi)$ memiliki tanda yang sama dan
$\alpha+\beta\equiv\gamma$, maka
$|\alpha|+|\beta|=|\gamma|$.
Terapkan hasil ini pada $\alpha=\measuredangle APB$,
...
Terapkan \ref{cor:half-plane}\textit{\ref{cor:half-plane:intersect}} untuk menunjukkan bahwa $O$ dan $A'$
terletak pada sisi yang sama terhadap~$(AB)$.
Dengan cara serupa, $O$ dan $B'$ terletak pada sisi yang sama terhadap~$(AB)$.
Terapkan kembali \ref{cor:half-plane}\textit{\ref{cor:half-plane:intersect}}.

\parbf{\ref{ex:signs-PXQ-PYQ}.}
Terapkan \ref{thm:signs-of-triug} pada $\triangle PQX$ dan $\triangle PQY$, lalu
terapkan \ref{cor:half-plane}\textit{\ref{cor:half-plane:angle}}.

\parbf{\ref{ex:chevinas}.} 
Kita dapat mengasumsikan bahwa $A'\z\ne B,C$ dan $B'\ne A, C$;
jika tidak, pernyataan tersebut berlaku dengan sendirinya.

Perhatikan bahwa $(BB')$ tidak memotong $[A'C]$.
Dengan menerapkan teorema Pasch (\ref{thm:pasch}) pada $\triangle AA'C$ dan $(BB')$, kita peroleh bahwa
$(BB')$ memotong $[AA']$; nyatakan titik potongnya dengan $M$.

Dengan cara yang sama, kita peroleh bahwa $(AA')$ memotong $[BB']$;
dengan kata lain, $M$ terletak pada $[AA']$ dan $[BB']$.

\parbf{\ref{ex:Z}.}
Andaikan $Z$ adalah titik potongnya.

Perhatikan bahwa $Z\ne P$ dan $Z\ne Q$.
Oleh karena itu, $Z\notin (PQ)$.

Tunjukkan bahwa $Z$ dan $X$ terletak pada sisi yang sama terhadap~$(PQ)$.
Ulangi argumen tersebut untuk menunjukkan bahwa $Z$ dan $Y$ terletak pada sisi yang sama terhadap~$(PQ)$.
Akibatnya, $X$ dan $Y$ terletak pada sisi yang sama terhadap $(PQ)$ --- sebuah kontradiksi.

\parbf{\ref{ex:intersecting-circles-3}.} Bagian ``hanya jika'' merupakan akibat dari ketaksamaan segitiga.
Untuk membuktikan bagian ``jika'',
perhatikan bahwa \ref{thm:abc} menjamin adanya sebuah segitiga dengan panjang sisi $r_1$, $r_2$, dan $d$.
Gunakan segitiga ini untuk menunjukkan bahwa terdapat titik $X$ sedemikian sehingga $O_1X=r_1$ dan $O_2X=r_2$, dengan $O_1$ dan $O_2$ sebagai pusat lingkaran yang bersesuaian.
...
Terapkan \ref{thm:isos} dua kali.

\parbf{\ref{ex:SMS}.} 
Misalkan $D$ dan $D'$ berturut-turut merupakan cerminan $C$ dan $C'$ terhadap $M$ dan $M'$.
Tunjukkan bahwa $\triangle BCD\z\cong \triangle B'C'D'$, lalu gunakan hasil ini untuk membuktikan bahwa $\triangle A' B' C'\z\cong\triangle A B C$.

\parbf{\ref{ex:isos-sides}.} \textit{(a)} Terapkan SAS.

\parit{(b)} Gunakan \textit{(a)} dan terapkan SSS.

\parbf{\ref{ex:ABC-motion}.}
Tanpa mengurangi keumuman, kita dapat mengasumsikan bahwa $X$ berbeda dari $A$, $B$, dan~$C$.
Tetapkan $f(X)=X'$; andaikan $X'\ne X$.

Perhatikan bahwa $AX=AX'$, $BX=BX'$, dan $CX=CX'$.
Berdasarkan SSS, diperoleh $\measuredangle ABX\z=\pm\measuredangle ABX'$.
Karena $X\ne X'$, diperoleh
$\measuredangle ABX\equiv - \measuredangle ABX'$.
Dengan cara yang sama, diperoleh
$\measuredangle CBX\equiv - \measuredangle CBX'$.
Dengan mengurangkan kedua identitas ini, diperoleh
$\measuredangle ABC\equiv -\measuredangle ABC$.
Simpulkan bahwa $\measuredangle ABC=0$ atau $\pi$.
Artinya, $\triangle ABC$ adalah segitiga degenerat --- suatu kontradiksi.

\parbf{\ref{ex:motion}.}
Konstruksikan tiga lingkaran yang berturut-turut berpusat di $A'$, $B'$, dan $C'$ serta berjari-jari $AX$, $BX$, dan $CX$.
Titik $X'$ merupakan titik persekutuan ketiga lingkaran tersebut.

%\parbf{\ref{ex:3-isos}.} Gunakan SAS untuk menunjukkan bahwa $\triangle ABA'\z\cong \triangle C'BC$, $\triangle BCB'\z\cong \triangle A'CA$, dan $\triangle CAC'\z\cong \triangle B'CB$.
...
Menurut Aksioma~\ref{def:birkhoff-axioms:2b} dan Teorema \ref{thm:straight-angle}, berlaku
$\measuredangle AOX\z+\measuredangle XOB\z\equiv\pi$; khususnya, $|\measuredangle AOX\z+\measuredangle XOB|\ge\pi$.
Simpulkan bahwa nilai $\measuredangle AOX$ dan $\measuredangle XOB$ keduanya tak negatif atau keduanya tak positif.

Pada masing-masing kasus, gunakan argumen seperti dalam \ref{ex:a+b==c} untuk menunjukkan bahwa
\columnbreak
$|\measuredangle AOX|\z+|\measuredangle XOB|=\pi$, lalu tarik kesimpulannya.

\parbf{\ref{ex:pbisec-side}.}
Andaikan $X$ dan $A$ terletak pada sisi yang sama dari~$\ell$.
...
Perhatikan bahwa $A$ dan $B$ terletak pada sisi yang berlawanan dari~$\ell$.
Oleh karena itu, menurut \ref{cor:half-plane},
$[AX]$ tidak memotong $\ell$,
sedangkan $[BX]$ memotong $\ell$;
misalkan $Y$ adalah titik potongnya.

Menurut ketaksamaan segitiga, $BX=AY\z+YX\z\ge AX$.
Karena $X\notin\ell$, menurut \ref{thm:perp-bisect} berlaku $AX\ne BX$.
Oleh karena itu, $BX> AX$.

Dengan demikian, kita telah membuktikan bagian ``jika''.
Untuk membuktikan bagian ``hanya jika'', pertukarkan $A$ dan $B$, lalu
ulangi argumen di atas.

\parbf{\ref{ex:side-angle}.}
Terapkan \ref{ex:pbisec-side}, \ref{ex:between}, dan \ref{thm:isos}.

\parbf{\ref{ex:pbisec-motion}.} Tunjukkan bahwa $FX=FY$, lalu terapkan \ref{thm:perp-bisect}.
...
Lihat gambar; data yang diberikan ditandai dengan garis tebal.
...
Pilih sembarang segitiga tak degenerat $ABC$.
Andaikan $\triangle \hat A \hat B\hat C$ menyatakan bayangannya setelah dikenai transformasi isometrik yang diberikan.

Jika $A\ne \hat A$, lakukan pencerminan terhadap garis sumbu~$[A\hat A]$.
Pencerminan ini memetakan $A$ ke~$\hat A$.
Misalkan $B'$ dan $C'$ masing-masing menyatakan bayangan pencerminan $B$ dan $C$.

Jika $B'\ne \hat B$, lakukan pencerminan terhadap garis sumbu~$[B'\hat B]$.
Pencerminan ini memetakan $B'$ ke~$\hat B$.
Perhatikan bahwa $\hat A\hat B=\hat AB'$;
dengan kata lain, $\hat A$ terletak pada garis sumbu tersebut.
Oleh karena itu, $\hat A$ merupakan titik tetap pencerminan ini.
Andaikan $C''$ menyatakan bayangan pencerminan~$C'$.

Terakhir, jika $C''\ne \hat C$, lakukan pencerminan terhadap $(\hat A\hat B)$.
Perhatikan bahwa $\hat A\hat C\z=\hat AC''$ dan $\hat B\hat C\z=\hat BC''$;
dengan kata lain, $(\hat A\hat B)$ merupakan garis sumbu $[C''\hat C]$.
Oleh karena itu, pencerminan ini memetakan $C''$ ke~$\hat C$.

Terapkan lema tiga paku (Latihan~\ref{ex:ABC-motion}) untuk menunjukkan bahwa komposisi pencerminan yang dikonstruksi berimpit dengan transformasi isometrik yang diberikan.

\parbf{\ref{ex:right-acute}.}
\textit{(a).} Terapkan \ref{lem:perp<oblique} dan \ref{ex:side-angle}.

\parit{(b).} Jika sudut di $C$ lurus, terapkan \ref{cor:degenerate=pi}.
Jika tidak, tunjukkan bahwa terdapat titik $D\in [AB]$ sedemikian sehingga $\angle ACD$ siku-siku.
Dengan menerapkan \textit{(a)}, tunjukkan bahwa sudut di $A$ lancip.
Ulangi argumen tersebut untuk~$B$.

\parbf{\ref{ex:obtuce}.}
Kita dapat mengandaikan bahwa $P \ne X$; jika tidak, pernyataannya sudah jelas.

Jika $\angle PXV$ siku-siku, pernyataan tersebut merupakan akibat dari \ref{lem:perp<oblique}.
Jika tidak, buatlah garis $(XY)$ yang tegak lurus terhadap~$(PX)$.
Tunjukkan bahwa $V$ dan $W$ terletak pada sisi yang berlawanan dari $(XY)$.

Tanpa mengurangi keumuman, andaikan $P$ dan $W$ terletak pada sisi yang berlawanan dari $(XY)$.
Misalkan $W'$ adalah titik potong $[PW]$ dan $(XY)$.
Tunjukkan dan gunakan fakta bahwa $PW>PW'>PX$.

\parbf{\ref{ex:PMQ}.}
Perhatikan bahwa $PM+MQ=P'M+MQ\z\ge P'Q$, lalu gunakan argumen seperti dalam \ref{lem:perp<oblique}.



\parbf{\ref{ex:inside-outside}.}
Terapkan \ref{ex:obtuce} dua kali pada $X$, $Y$, $P$, dan titik pusat.
...
\parbf{\ref{ex:chord-perp}.} Terapkan \ref{thm:perp-bisect}.

\parbf{\ref{ex:center}.}
Lihat gambar; lingkaran yang diberikan digambar dengan garis tebal.

\parbf{\ref{ex:two-circ}.} Gunakan \ref{ex:chord-perp} dan \ref{perp:ex+un}.

}

\parbf{\ref{ex:tangent-circles}}; \textit{(\ref{ex:tangent-circles:a}).}
Andaikan $Q$ adalah titik lain pada kedua lingkaran.
Tunjukkan dan gunakan fakta bahwa $Q$ merupakan bayangan pencerminan $P$ terhadap~$(OO')$.

\parit{(\ref{ex:tangent-circles:b}).} Terapkan \textit{(\ref{ex:tangent-circles:a})} dan \ref{lem:tangent}.

\parit{(\ref{ex:tangent-circles-2}).} Terapkan \textit{(\ref{ex:tangent-circles:a})} dan \ref{cor:degenerate-trig}.

\parbf{\ref{ex:tangent-circles-3}.}
Misalkan $A$ dan $B$ adalah titik-titik potongnya.
Perhatikan bahwa pusat kedua lingkaran terletak pada garis sumbu ruas~$[AB]$.
...
\parbf{\ref{ex:mid-triangle}.} Terapkan kriteria kesebangunan SAS untuk menunjukkan bahwa
$\triangle AC'B'\sim\triangle ABC$, $\triangle C'BA'\sim\triangle ABC$, dan $\triangle B'A'C\sim\triangle ABC$.
Tunjukkan bahwa dalam setiap kasus, koefisien kesebangunannya adalah $\tfrac12$.
Simpulkan bahwa $A'B'=\tfrac12\cdot AB$, $B'C'=\tfrac12\cdot BC$, dan $C'A'\z=\tfrac12\cdot CA$.
Terapkan kriteria kesebangunan SSS.

\parbf{\ref{ex:k*triangle}.} Terapkan kriteria kesebangunan SAS untuk menunjukkan bahwa
$\triangle OA'B'\sim\triangle OAB$,
$\triangle OB'C'\sim\triangle OBC$,
dan $\triangle OC'A'\sim\triangle OCA$.
Tunjukkan bahwa dalam setiap kasus, koefisien kesebangunannya adalah $k$.
Simpulkan bahwa $A'B'=k\cdot AB$, $B'C'=k\cdot BC$, dan $C'A'\z=k\cdot CA$.
Terapkan kriteria kesebangunan SSS.
...
Perhatikan bahwa faktor ini bernilai sama bagi sebarang dua segitiga tak degenerat yang memiliki satu sisi bersama.
Terapkan pengamatan ini pada suatu rantai segitiga.

\parbf{\ref{ex:pyth}.}
Gunakan fakta bahwa $\triangle ADC\sim \triangle CDB$.

\parbf{\ref{ex:pyth-conv}.}
Terapkan teorema Pythagoras (\ref{thm:pyth}) dan kriteria kekongruenan SSS.

\parbf{\ref{ex:two-pairs-sim}.}
Berdasarkan kriteria kesebangunan AA (\ref{prop:sim}), $\triangle AYC\z\sim \triangle BXC$.
Simpulkan bahwa
$\frac{YC}{AC}=\frac{XC}{BC}$.
Terapkan kriteria kesebangunan SAS untuk menunjukkan bahwa $\triangle ABC\z\sim \triangle YXC$.

Dengan cara serupa, terapkan AA dan fakta bahwa sudut-sudut bertolak belakang sama besar untuk membuktikan bahwa $\triangle AZX\sim \triangle BZY$, lalu gunakan SAS untuk menunjukkan bahwa $\triangle ABZ\z\sim \triangle YXZ$.

\parbf{\ref{ex:ABC+D}.}
Tunjukkan bahwa $\triangle ABC\sim \triangle CBD$, kemudian gunakan fakta tersebut.

\parbf{\ref{ex:right-perp-bi}.}
Tunjukkan bahwa $\triangle AQM\sim \triangle ABC\z\sim\triangle PBM$, kemudian gunakan fakta tersebut.
...
\parbf{\ref{ex:perp-perp}}; \textit{(\ref{ex:perp-perp:a}).}
Terapkan \ref{thm:parallel} untuk menunjukkan bahwa $n\nparallel \ell$, sehingga $n$ dan $\ell$ berpotongan di satu titik, sebutlah~$P$.
Gambarlah garis $\ell'\perp n$ yang melalui $P$.
Berdasarkan \ref{prop:perp-perp}, $\ell'\parallel m$.
Terapkan kembali \ref{thm:parallel} untuk menunjukkan bahwa $\ell= \ell'$.

\parit{(\ref{ex:perp-perp:b}).} Dengan menerapkan \textit{(\ref{ex:perp-perp:a})}, kita memperoleh $k\perp m$.
Tinggal menerapkan \ref{prop:perp-perp}.

\parbf{\ref{ex:construction-parallel}.}
Ulangi konstruksi dalam \ref{ex:construction-perpendicular} sebanyak dua kali.

\parbf{\ref{ex:reflections}.}
Pertama, gunakan kembali argumen dalam \ref{ex:2-reflections}.
Selanjutnya, tunjukkan bahwa komposisi dua transformasi isometrik tak langsung merupakan transformasi isometrik langsung, dan gunakan fakta bahwa pencerminan terhadap suatu garis bersifat tak langsung (lihat \ref{cor:reflection+angle}).
...
Terapkan Aksioma~\ref{def:birkhoff-axioms:2b}, \ref{thm:parallel-2}, dan \ref{ex:2a=0}.

\parbf{\ref{ex:smililar+parallel}.}
Karena $\ell\parallel (AC)$, garis tersebut tidak dapat memotong $[AC]$.
Berdasarkan teorema Pasch (\ref{thm:pasch}), $\ell$ harus memotong sisi lain dari $\triangle ABC$.
Dengan demikian, $\ell$ memotong $[BC]$; lambangkan titik potongnya dengan $Q$.


Gunakan sifat transversal (\ref{thm:parallel-2}) untuk menunjukkan bahwa $\measuredangle BAC= \measuredangle BPQ$.
Argumen yang sama menunjukkan bahwa $\measuredangle ACB\z= \measuredangle PQB$; tinggal menerapkan kriteria kesebangunan AA.



\parbf{\ref{ex:trisection}.}
Andaikan kita perlu membagi ruas $[AB]$ menjadi tiga bagian sama panjang.
Konstruksikan garis $\ell\ne (AB)$ yang memuat empat titik $A,C_1,C_2, C_3$
sedemikian sehingga $C_1$ dan $C_2$ membagi $[AC_3]$ menjadi tiga bagian sama panjang.
Gambarlah garis $(BC_3)$
dan garis-garis sejajar yang masing-masing melalui $C_1$ dan~$C_2$.
Titik-titik potong kedua garis ini dengan $(AB)$ membagi ruas $[AB]$ menjadi tiga bagian sama panjang.

\parbf{\ref{ex:|3sum|}.}
Jika $\triangle ABC$ degenerat, salah satu ukuran sudutnya adalah $\pi$, sedangkan dua lainnya adalah~$0$.
Dengan demikian, pernyataan terbukti.

Andaikan $\triangle ABC$ tak degenerat.
Tetapkan $\alpha\z=\measuredangle CAB$, $\beta=\measuredangle ABC$, dan $\gamma=\measuredangle BCA$.

Berdasarkan \ref{thm:signs-of-triug},
kita dapat menganggap bahwa $0<\alpha,\beta,\gamma<\pi$.
Dengan demikian,
\[0<\alpha+\beta+\gamma<3\cdot\pi.
\eqlbl{eq:|3|<3pi}\]

Berdasarkan \ref{thm:3sum},
$$\alpha+\beta+\gamma\equiv\pi.\eqlbl{eq:|3|==pi}$$

Dari \ref{eq:|3|<3pi} dan \ref{eq:|3|==pi}, pernyataan tersebut langsung diperoleh.

\parbf{\ref{ex:pent}.}
Terapkan \ref{thm:3sum} atau \ref{ex:|3sum|} dua kali dan \ref{thm:isos} tiga kali.

\parbf{\ref{ex:right-isos}.}
Terapkan \ref{thm:isos} dua kali dan \ref{thm:3sum} satu kali.
...
Terapkan \ref{thm:3sum} pada $\triangle ABC$ dan $\triangle BDA$.

\parbf{\ref{ex:4parallels}.}
Misalkan $M$ menyatakan pusat simetri $\square ABCD$;
titik ini ada berdasarkan \ref{lem:parallelogram}.
Misalkan $P'$ merupakan hasil pencerminan $P$ terhadap $M$.
Tunjukkan dan gunakan fakta bahwa $a=(AP')$, $b=(BP')$, $c=(CP')$, dan $d=(DP')$.

\parbf{\ref{ex:romb}.}
Karena $\triangle ABC$ sama kaki, $\measuredangle CAB\z=\measuredangle BCA$.

Berdasarkan SSS, $\triangle ABC\cong \triangle CDA$.
Dengan demikian,
$\pm\measuredangle DCA= \measuredangle BCA=\measuredangle CAB$.

Karena $D\ne B$, tanda dalam rumus terakhir harus ``$-$''.
Gunakan sifat transversal (\ref{thm:parallel-2}) untuk menunjukkan bahwa $(AB)\parallel (CD)$. Ulangi argumen tersebut untuk menunjukkan bahwa $(AD)\z\parallel(BC)$.

\parbf{\ref{ex:rectangle}.}
Berdasarkan \ref{lem:parallelogram} dan SSS,
$AC=BD$
jika dan hanya jika
$\measuredangle ABC=\pm \measuredangle BCD$.
Berdasarkan sifat transversal~(\ref{thm:parallel-2}),
...
\parbf{\ref{ex:romb2}.}
Ambil suatu jajar genjang $ABCD$.
Berdasarkan \ref{lem:parallelogram},
diagonal-diagonal $[AC]$ dan $[BD]$ memiliki titik tengah yang sama; nyatakan titik itu dengan~$M$.

Gunakan SSS dan \ref{lem:parallelogram} untuk menunjukkan bahwa
$AB=AD\ \iff\ \triangle AMB
\cong
...
Tunjukkan dan gunakan fakta bahwa $\triangle CZY\sim\triangle YXB$.

\parbf{\ref{ex:coordinates}.} \textit{(a).} Gunakan ketunggalan garis sejajar (\ref{thm:parallel}).

\parit{(b)} Gunakan \ref{lem:parallelogram} dan teorema Pythagoras (\ref{thm:pyth}).

\parbf{\ref{ex:abc}.}
Tetapkan $A=(0,0)$, $B=(c,0)$, dan $C=(x,y)$.
Jelas bahwa $AB=c$,
$AC^2=x^2+y^2$ dan $BC^2\z=(c-x)^2+y^2$.

Tinggal menunjukkan bahwa terdapat pasangan bilangan real $(x,y)$
...
\right.
$$
jika $0<a\le b\le c\le a+b$.

\parbf{\ref{ex:line-coord}.}
Perhatikan bahwa $MA=MB$ jika dan hanya jika
\[(x-x_A)^2+(y-y_A)^2=(x-x_B)^2+(y-y_B)^2,\]
dengan $M=(x,y)$.
Untuk membuktikan bagian pertama, sederhanakan persamaan ini.
Untuk bagian-bagian lainnya, gunakan fakta bahwa sebarang garis merupakan garis sumbu suatu ruas garis.

\parbf{\ref{ex:circle-coord}.} Tulislah ulang persamaan itu dalam bentuk berikut, lalu pikirkan maknanya:
\[(x+\tfrac a2)^2+(y+\tfrac b2)^2=(\tfrac a2)^2+(\tfrac b2)^2-c.\]


\parbf{\ref{ex:apolonnius}.}
Kita dapat memilih koordinat sedemikian sehingga $B=(0,0)$ dan $A=(a,0)$ untuk suatu $a>0$.
Jika $M=(x,y)$, persamaan $AM=k\cdot BM$ dapat ditulis dalam koordinat sebagai
\[k^2\cdot(x^2+y^2)=(x-a)^2+y^2.\]
Tinggal menulis ulang persamaan ini seperti dalam \ref{ex:circle-coord}.

\parbf{\ref{ex:apolonnius-construction}.}
Andaikan $M\notin(AB)$.
Tunjukkan dan gunakan fakta bahwa titik-titik $P$ dan $Q$ yang dikonstruksi pada gambar berikut terletak pada lingkaran Apollonius.
...
Terapkan \ref{thm:circumcenter} dan \ref{thm:perp-bisect}.

\parbf{\ref{ex:orthic-4}.}
Perhatikan bahwa $(AC)\perp (BH)$ dan $(BC)\z\perp (AH)$, lalu terapkan \ref{thm:orthocenter}.

(Perhatikan bahwa masing-masing titik $A,B,C,H$ merupakan ortosenter dari segitiga yang dibentuk oleh tiga titik lainnya; susunan empat titik $A,B,C,H$ semacam itu disebut \index{sistem ortosentrik}\emph{sistem ortosentrik}.)

\parbf{\ref{ex:orthic-sim}.}
Tunjukkan bahwa $\triangle AA'C \sim \triangle BB'C$.
Gunakan hasil ini untuk menunjukkan bahwa $\triangle ABC \sim \triangle A'B'C$.
Ulangi argumen tersebut untuk $\triangle AB'C'$ dan $\triangle A'BC'$.

\parbf{\ref{ex:midle}.}
Gunakan gagasan dari bukti \ref{thm:centroid}
untuk menunjukkan bahwa $(XY)\z\parallel (AC)\z\parallel (VW)$ dan
$(XW)\z\parallel (BD)\z\parallel (YV)$.

\parbf{\ref{ex:euler-line}.}
Pelajari homoteti.

Dengan menerapkan \ref{thm:centroid}, tunjukkan bahwa homoteti yang berpusat di $M$ dengan faktor $-\tfrac{1}{2}$ memetakan $\triangle ABC$ ke segitiga medialnya $\triangle A'B'C'$.
Tunjukkan bahwa homoteti tersebut juga memetakan ortosenter $\triangle ABC$ ke ortosenter $\triangle A'B'C'$.

Dengan argumen seperti dalam \ref{thm:orthocenter}, tunjukkan bahwa ortosenter $\triangle A'B'C'$ berimpit dengan pusat lingkaran luar $\triangle ABC$.
Tarik kesimpulan.

\parbf{\ref{ex:perp-bisectors}.}
Misalkan $(BX)$ dan $(BY)$ masing-masing merupakan garis bagi dalam dan garis bagi luar $\angle ABC$.
Maka
\begin{align*}
2\cdot \measuredangle XBY&\equiv2\cdot \measuredangle XBA+2\cdot \measuredangle ABY\equiv
\\
&\equiv
\measuredangle CBA+\pi+ \measuredangle ABC\equiv
\\
...
Jika $E$ adalah titik potong $(BC)$
dengan garis bagi luar $\angle BAC$, maka
$\frac{AB}{AC}\z=\frac{EB}{EC}$.
Hal ini dapat dibuktikan dengan cara yang sama seperti \ref{lem:bisect-ratio}.

\parbf{\ref{ex:bisect=median}.}
Terapkan \ref{lem:bisect-ratio}.
Lihat juga penyelesaian \ref{ex:abs-bisect=median}.

\parbf{\ref{ex:bisector-parallel}.}
Terapkan \ref{thm:isos}, \ref{thm:parallel-2}, dan \ref{lem:parallelogram}.

\parbf{\ref{ex:2x=b+c-a}.}
Misalkan $I$ adalah pusat lingkaran dalam.
Dengan kriteria SAS, diperoleh $\triangle AIZ\z\cong\triangle AIY$.
Oleh karena itu, $AY=AZ$.
Dengan cara yang sama, diperoleh $BX=BZ$ dan $CX=CY$.
Dengan demikian, hasil yang diminta terbukti.

\parbf{\ref{ex:orthic-triangle}.}
Gunakan argumen seperti dalam \ref{ex:orthic-sim} untuk menunjukkan bahwa $\triangle A'B'C\z\sim \triangle ABC\z\sim\triangle AB'C'$.
Simpulkan bahwa $(BB')$ membagi dua $\angle A'B'C'$.

Jika $\triangle ABC$ tumpul, maka ortosenternya berimpit dengan salah satu \index{eksenter}\emph{eksenter} dari $\triangle A'B'C'$;
yaitu,
titik potong dua garis bagi luar dan satu garis bagi dalam dari $\triangle A'B'C'$.

\parbf{\ref{ex:bisector-incenter}.}
Terapkan \ref{lem:bisect-ratio} dua kali.
Gunakan identitas yang diperoleh untuk menunjukkan bahwa garis bagi sudut di $C$ melalui $I$.
...
\parit{(b)} If $P$ is inside $\Gamma$, then $P$ lies between $X$ and $X'$ and between $Y$ and $Y'$.
...
Observe that the points $A$, $A'$, $B$, and $B'$ lie on one circle.
...
are the images (coordinates) of the two points in $\ell\cap\Gamma$;
note that $x\ne y$.
...
Observe that the two circles that are tangent at $P$ become parallel lines and
...
to show that a circline $\Gamma$ passing thru $P$, $P_1$, and $P_2$ is a solution.
...
If $C'\z\ne C''$ apply \ref{prop:2sum} to $\triangle A'C'C''$ and try to reach a contradiction.
...
Note that the center of the Euclidean circle containing $\ell$ lies at the intersection of the lines tangent to the absolute at the ideal points of~$\ell$.

\parbf{\ref{ex:1ideal-line-unique}.}
Assume $A$ is an ideal point of the h-line $\ell$
and $P\in \ell$.
Let $P'$ be the inverse of $P$ across the absolute.
...
(We could also move $Q$ to the center of the absolute.
...
By \ref{thm:main-observ}, we can assume that $O$ is the center of the absolute.
...
and it approaches $\tfrac\pi4$ as $CD_h\to\infty$.
...
Denote by $R$ and $R'$ the h-footpoints of $P$ and $P'$ on $m$.
...
Use that $\alpha(\Gamma)=\Gamma$ to show that $(\begin{smallmatrix}a&b\\c&d\end{smallmatrix})\cdot (\begin{smallmatrix}a&c\\b&d\end{smallmatrix})\z=(\begin{smallmatrix}1&0\\0&1\end{smallmatrix})$.
...
Show that we can compose $\alpha$ with several inversions and reflections to obtain a bijection $\alpha'$ such that $\alpha'(\infty)=\infty$ and $\alpha(\Gamma)=\Gamma$ for some circle $\Gamma$.
...
and (2) the ideal point given by the pencil of lines $b\cdot x-a\cdot y=c$ (for different values of $c$) corresponds to the line defined by $a\cdot x+b\cdot y=0$.
...
\parit{(\ref{ex:s-medians:b}).}
Notice that the medians of the spherical triangle $ABC$
map to the medians of the Euclidean triangle $A'B'C'$.
...
If the extension of $m$ is passing thru $Z$, then the inversion across $\Gamma$ exchanges the ideal points of~$\hat\ell$.
...
In the Euclidean plane, the circle $\Gamma$ is tangent to $k$;
that is, $T$ is the only point of intersection of $\Gamma$ and $k$.
...
Assume that one can construct a bisector of $\angle AOB$, where $A=(1,0)$, $O=(0,0)$, and $B=(1,1)$.
Let $D$ be the point of intersection of the bisector with the line $(AB)$.
Use \ref{lem:bisect-ratio} to show that $D$ is irrational.
...
Let $h_A$ and $h_C$ denote the distances from $(BD)$ to
...
\\
\area(\solidtriangle DAM)&=\tfrac12\cdot h_A\cdot DM.
\end{align*}
Therefore
\begin{align*}
...
If $X$ is a point inside $\triangle ABC$, then $\solidtriangle ABC$ is subdivided into $\solidtriangle ABX$, $\solidtriangle BCX$, and $\solidtriangle CAX$.
Therefore
\begin{align*}
...
Multiplying the identities, we get the last statement.
...
\quad
n\to\infty.$$
Next show that $\area\mathcal{Q}_n<100$ for any large $n$.
